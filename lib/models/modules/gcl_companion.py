...
class GCL_Companion(nn.Module):
    ...
    def forward(self, input_img, seg_map):
        ...
        if self.n_channels == 1:
            ...
            x0_0 = torch.mul(input_img[:, 0, :, :].expand(b, self.num_classes, h, w), seg_map[b, self.num_classes, h, w])
            x0_1 = torch.mul(input_img[:, 1, :, :].expand(b, self.num_classes, h, w), seg_map[b, self.num_classes, h, w])
            x0_2 = torch.mul(input_img[:, 2, :, :].expand(b, self.num_classes, h, w), seg_map[b, self.num_classes, h, w])
