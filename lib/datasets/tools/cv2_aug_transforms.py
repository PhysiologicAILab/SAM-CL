...
# # from six.moves import collections_abc as collectionsAbc
...
class _BaseTransform(object):
    ...
    def __call__(self, img, **kwargs):
        ...
        # print(vars(collectionsAbc))
        # exit()

        data_dict = collectionsAbc.defaultdict(lambda: None)
    ...
    def _process(self, img, data_dict, skip_condition, *args, **kwargs):
        ...
        ret_dict = collectionsAbc.defaultdict(lambda: None)
...
class RandomCrop(_BaseTransform):
    ...
    def __init__(self, crop_size, crop_ratio=0.5, method='random', grid=None, allow_outside_center=True):
        ...
        elif isinstance(crop_size, collectionsAbc.abc.Iterable) and len(crop_size) == 2:
            self.size = crop_size
        else:
